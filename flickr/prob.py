...
# Output to files
with open("flickr_learn/attribute.txt", "w") as f_attr:
    f_attr.write("n=%d\n" % lwcc.vcount())
    f_attr.write("m=%d\n" % lwcc.ecount())

lwcc.write_ncol("flickr_learn/graph_ic.inf")
with open("flickr_learn/graph_ic_nm.inf", "w") as f_graph_nm:
    f_graph_nm.write("%d %d\n" % (lwcc.vcount(), lwcc.ecount()))
    with open("flickr_learn/graph_ic.inf", "r") as f_tmp:
