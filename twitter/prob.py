...
# Output to files
with open("twitter_learn/attribute.txt", "w") as f_attr:
    f_attr.write("n=%d" % lwcc.vcount())
    f_attr.write("m=%d" % lwcc.ecount())

lwcc.write_ncol("twitter_learn/graph_ic.inf")
with open("twitter_learn/graph_ic_nm.inf", "w") as f_graph_nm:
    f_graph_nm.write("%d %d\n" % (lwcc.vcount(), lwcc.ecount()))
    with open("twitter_learn/graph_ic.inf", "r") as f_tmp:
